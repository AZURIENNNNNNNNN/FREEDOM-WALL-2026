...
function Watermark({ viewerEmail }) {
  const label = viewerEmail ? viewerEmail.split("@")[0] : "archer";
  return (
    <div className="watermark" aria-hidden="true">
      {Array.from({ length: 12 }).map((_, i) => (
        <span key={i} className="wm-text">viewed by {label}</span>
      ))}
    </div>
  );
}

function PostCard({ post, isAdmin, onVote, onDelete, viewerEmail }) {
      ...
      <div className={`modal ${shake ? "shake" : ""}`}>
        <div className="modal-crest">
          <div className="crest-ring">
            <img src="https://files.catbox.moe/k97am8.png" style={{ width: "60px", height: "60px", borderRadius: "50%", objectFit: "cover" }} />
          </div>
        </div>
        {step === "code" ? (
          <>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          --green: #1a472a; --green-mid: #2d6a4f; --green-light: #40916c;
          --green-pale: #d8f3dc; --green-wash: #f0faf4;
          --gold: #c9a84c; --gold-light: #f0d080; --gold-pale: #fdf6e3;
          --white: #ffffff; --off: #f7f9f8; --text: #0e2318; --text-mid: #2d5a3d;
          --muted: #6b8f74; --border: #c8e6d4; --red: #c0392b; --amber: #d4880f;
          --shadow: 0 4px 24px rgba(26,71,42,0.12); --shadow-lg: 0 12px 48px rgba(26,71,42,0.20);
        }
        body { background: var(--off); font-family: 'Source Sans 3', sans-serif; color: var(--text); min-height: 100vh; }
        body::before { content: ''; position: fixed; top: 0; left: 0; right: 0; height: 340px; background: linear-gradient(160deg, var(--green) 0%, var(--green-mid) 60%, var(--green-light) 100%); clip-path: polygon(0 0, 100% 0, 100% 68%, 0 100%); z-index: 0; }
        .app { position: relative; z-index: 1; max-width: 700px; margin: 0 auto; padding: 0 1rem 5rem; }
        .site-header { padding: 2.5rem 0 2rem; display: flex; align-items: flex-end; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
        .header-left { display: flex; align-items: center; gap: 1.1rem; }
        .header-logo { width: 58px; height: 58px; border-radius: 50%; object-fit: cover; box-shadow: 0 4px 16px rgba(201,168,76,0.5); flex-shrink: 0; }
        .header-text h1 { font-family: 'Playfair Display', serif; font-size: 2.2rem; font-weight: 900; color: var(--white); letter-spacing: -0.02em; line-height: 1; text-shadow: 0 2px 12px rgba(0,0,0,0.25); }
        ...
        .warning-banner { background: #7a1a1a; border-radius: 12px; padding: 0.65rem 1.2rem; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.75rem; }
        .warning-banner-text { font-size: 0.76rem; font-weight: 700; color: #fecaca; letter-spacing: 0.04em; line-height: 1.5; }
        ...
        .watermark { position: absolute; inset: 0; display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 1.5rem 2rem; padding: 0.5rem; pointer-events: none; z-index: 0; overflow: hidden; transform: rotate(-20deg) scale(1.3); }
        ...
        .btn-primary { background: linear-gradient(135deg, var(--green), var(--green-mid)); color: var(--white); border: none; border-radius: 50px; padding: 0.72rem 1.5rem; font-family: 'Source Sans 3', sans-serif; font-weight: 700; font-size: 0.92rem; cursor: pointer; width: 100%; margin-top: 0.5rem; transition: all 0.15s; box-shadow: 0 4px 16px rgba(26,71,42,0.25); }
        ...
        .crest-ring { width: 72px; height: 72px; border-radius: 50%; background: var(--green-wash); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 20px rgba(26,71,42,0.3), 0 0 0 4px var(--gold); overflow: hidden; }
      ...
      <div className="app">
        <div className="site-header">
          <div className="header-left">
            <img src="https://files.catbox.moe/k97am8.png" className="header-logo" alt="DLSU Archer" />
        ...
        {session && <>
          <div className="warning-banner">
            ...
            <span className="warning-banner-text">Screenshotting and sharing posts is a violation of this space's trust. Posts are watermarked — leaks can be traced back to you.</span>
          </div>

          <div className="banner">
            <span style={{ fontSize: "1rem" }}>🏹</span>
            <span className="banner-text">Animo La Salle! — Say it freely. You post anonymously.</span>
          </div>
